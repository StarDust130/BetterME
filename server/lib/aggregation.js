...
export const allDayStats = async (
  clerkID,
  dateFilter,
  page = 1,
  limit = 10,
  ...
  try {
    ...
    const dayTaskStats = await DayTask.aggregate([
      {
        $match: {
          clerkID,
          ...dateFilter, // Filter by date (e.g., completed date)
        },
      },
      {
        $sort: { [sortBy]: sortOrder === "asc" ? 1 : -1 }, // Sort by the chosen field
      },
      {
    ...
    // Aggregate for habit: Habits completed on the given day
